var hoverEffect = function (e) {
  ...
  if (i)
    if (r && o && a) {
      ...
      x.setPixelRatio(window.devicePixelRatio), x.setClearColor(16777215, 0), x.setSize(i.offsetWidth, i.offsetHeight), i.appendChild(x.domElement);
      var y = function () {
          x.render(g, h)
        },
      ...
      var F = w.load(r, y),
        E = w.load(o, y),
        M = w.load(a, y);
      M.wrapS = M.wrapT = t.RepeatWrapping, F.magFilter = E.magFilter = t.LinearFilter, F.minFilter = E.minFilter = t.LinearFilter;
      var P = new t.ShaderMaterial({
        ...
        R = new t.Mesh(L, P);
      g.add(R), c && (i.addEventListener("mouseenter", U), i.addEventListener("touchstart", U, {passive: true}), i.addEventListener("mouseleave", C), i.addEventListener("touchend", C)), window.addEventListener("resize", function (e) {
        x.setSize(i.offsetWidth, i.offsetHeight)
      }), this.next = U, this.previous = C
